    ...
    public function data(Request $request)
    {
        ...
        return DataTables::of($products)
            ->filter(function ($query) use ($request) {
                // Handle global search manually
                if ($request->has('search') && !empty($request->search['value'])) {
                    $search = $request->search['value'];
                    
                    $query->where(function ($q) use ($search) {
                        // Search on actual table columns, not aliases
                        $q->where('p.name', 'LIKE', "%{$search}%")
                        ->orWhere('p.sku', 'LIKE', "%{$search}%")
                        ->orWhere('c.name', 'LIKE', "%{$search}%")
                        ->orWhere('b.name', 'LIKE', "%{$search}%");
                        
                        // For business_location (GROUP_CONCAT), we need a different approach
                        // You can either remove this from search or use a HAVING clause
                        // Since it's a GROUP_CONCAT, we can't easily search it in WHERE
                        // $q->orHaving('business_location', 'LIKE', "%{$search}%");
                    });
                }
            })
