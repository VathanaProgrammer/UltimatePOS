...
class SaleOnlineController extends Controller
{
    ...
    public function getData(Request $request)
    {
        try {
            ...
            return DataTables::of($orders)
                ...
                ->addColumn('mobile', fn($order) => optional($order->api_user->contact)->mobile ?? ($order->currentAddress->phone ?? 'N/A'))
                ...
                ->addColumn('stauts', fn($order) => '<span class="px-1 py-0 rounded" style="background-color: #f59e0b; color: white;">' . $order->stauts . '</span>')
                ->addColumn('shipping_status', fn($order) => '<span class="px-1 py-0 rounded" style="background-color: #f59e0b; color: white;">' . ($order->status ?? 'pending') . '</span>')
                ->addColumn('details', function ($order) {
                    // LOG the items for debugging
                    $itemsArray = $order->order_online_details->map(fn($d) => [
   ...
   public function create_sale_order_from_online(Request $request)
{
    DB::beginTransaction();
    try {
        ...
        $transaction = [
            ...
            'location_id' => $order->location_id ?? 1 | 2,
