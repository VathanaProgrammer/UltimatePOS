...
let shippingDropzone = null;
let invoiceDropzone = null;
let shippingFiles = [];
let invoiceFiles = [];


function initShippingDropzone() {
    const $dropzone = $("#shipping_documents_dropzone");
    if ($dropzone.length && !shippingDropzone) {
        shippingDropzone = new Dropzone($dropzone[0], {
            url: $('#media_upload_url').val(),
            paramName: 'file',
            uploadMultiple: true,
            autoProcessQueue: false,
            ...
            params: { // <<< critical params included here
                'model_id': $('#model_id').val(),
                'model_type': $('#model_type').val(),
                'model_media_type': $('#model_media_type').val()
            },
...
function initInvoiceDropzone() {
    $(document).on("change", "#include_invoice_checkbox", function() {
        ...
        if ($(this).is(":checked")) {
            $dropzone.show();
            $("#invoice_status").text("Select invoice file(s).");

            if (!invoiceDropzone && $dropzone.length) {
                invoiceDropzone = new Dropzone($dropzone[0], {
                    url: '/invoice/upload-temp',
                    ...
                    init: function() {
                        ...
                        this.on("removedfile", file => {
                            invoiceFiles = invoiceFiles.filter(f => f !== file);
                            $("#invoice_status").text(invoiceFiles.length ? "Files ready." : "No invoice selected.");
                        });
                    }
                });
            }
...
$(document).on('submit', 'form#edit_shipping_form', function(e) {
    ...
    formData.append('model_media_type', $('#model_media_type').val());
